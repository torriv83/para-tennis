{{--
    ...
    Required variables:
    - $standings: array of standing rows (each holds the competitor under $competitorKey + stats)
    - $competitorKey: 'player' or 'team' — the row key holding the model (must expose displayName())
    - $title: section heading
    - $label: competitor column header
    - $keyPrefix: wire:key prefix to keep rows unique across tables
--}}
            ...
            <table class="w-full text-sm sm:text-base">
                <thead>
                    <tr class="border-b border-white/10 text-left text-[10px] uppercase tracking-wide text-text-muted sm:text-xs">
                        ...
                        <th class="pb-2 pr-2 sm:pb-3 sm:pr-4">{{ $label }}</th>
                ...
                <tbody>
                    @foreach($standings as $index => $standing)
                        ...
                        @endphp
                        <tr wire:key="{{ $keyPrefix }}-{{ $standing[$competitorKey]->id }}" class="border-b border-white/5">
                            <td class="py-2 pr-1 text-xs text-text-secondary sm:py-3 sm:pr-3 sm:text-sm">{{ $index + 1 }}</td>
                            <td class="max-w-[120px] truncate py-2 pr-2 text-xs font-medium sm:max-w-none sm:py-3 sm:pr-4 sm:text-sm">{{ $standing[$competitorKey]->displayName() }}</td>
