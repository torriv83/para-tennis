{{--
    Doubles Match Card Component

    Required variables:
    - $game: The doubles game model (is_doubles = true)
    - $tournament: The tournament model (for date constraints)
--}}

<div wire:key="doubles-game-{{ $game->id }}-{{ $game->scheduled_at?->timestamp ?? 'none' }}-{{ $game->completed ? 'done' : 'pending' }}" class="rounded-lg border border-white/5 bg-surface-light p-3 sm:p-4">
    {{-- Schedule Row --}}
                ...
                @if(!$game->completed)
                    <div class="relative sm:hidden">
                        ...
                        <div
                            ...
                            class="absolute right-0 top-full z-10 mt-2 w-56 rounded-lg border border-white/10 bg-surface p-2 shadow-xl"
                        >
                            <button
                            ...
                            <div class="relative">
                                ...
                                >
                                    <p class="px-3 py-1 text-xs text-text-muted">{{ __('messages.select_winning_team') }}</p>
                                    <button
    ...
    <div class="flex items-center gap-2 sm:gap-4">
        <div class="flex flex-1 items-center gap-2">
            ...
            @canEditResults($tournament)
                <button
                    ...
                    class="shrink-0 cursor-pointer rounded p-1 text-text-muted transition hover:bg-surface hover:text-text-secondary"
                    title="{{ __('messages.swap_teams') }}"
                >
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @include('livewire.tournament.partials.score-entry', ['game' => $game, 'variant' => 'doubles'])
