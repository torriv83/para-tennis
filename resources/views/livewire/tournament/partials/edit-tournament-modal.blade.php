        ...
        >
            ...
            <div class="relative max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl border border-white/10 bg-surface p-4 shadow-xl sm:p-6">
                ...
                <form wire:submit="updateTournament" class="space-y-4 sm:space-y-5">
                    ...
                    {{-- Include Doubles --}}
                    <div x-data="{ editHasDoubles: @entangle('editHasDoubles') }">
                        <label class="flex cursor-pointer items-center gap-3">
                            <input
                                type="checkbox"
                                x-model="editHasDoubles"
                        ...
                        {{-- Doubles Format --}}
                        <div x-show="editHasDoubles" x-cloak class="mt-4 pl-8">
                            <label class="mb-3 block text-sm font-medium text-text-secondary">{{ __('messages.doubles_format') }}</label>
                            <div class="grid gap-3 sm:grid-cols-2">
                                <label class="group cursor-pointer">
                                    <input type="radio" wire:model="editDoublesFormat" value="round_robin" class="peer hidden">
                                ...
                                <label class="group cursor-pointer">
                                    <input type="radio" wire:model="editDoublesFormat" value="round_robin_finals" class="peer hidden">
                                    <div class="rounded-xl border border-white/10 bg-background/30 p-3 transition peer-checked:border-secondary peer-checked:bg-secondary/10 group-hover:border-white/20">
                                        <span class="font-medium">{{ __('messages.round_robin_finals') }}</span>
                                        <p class="text-sm text-text-muted">{{ __('messages.round_robin_finals_short') }}</p>
                                    </div>
